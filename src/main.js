...
//Load dynamic form for the relevant clicking layer properties
//STILL HACKY
//DESIGNED TO DO THIS JUST ONCE
//REACT SHOULD DO THE TWO WAY BINDING BETWEEN the GeoJSON object and the forms. 

generateFormFromGeoJSON(pg_test.getProperties,'info');


//Select and modify

//TO DO HIGHLIGHT FEATURES WHEN SELECTED
  ...
  */
  ...
  function showInfo(event) {
    const features = pg_test.getFeatures(event.pixel).then (function(features){
      ...
      generateFormFromGeoJSON(properties,'info');
    ...
    */
    ...
    function generateFormFromGeoJSON(geojsonObject, containerId) {
      /*React version might want something 
      *
      */
      ...
      for (const [key, value] of Object.entries(geojsonObject).sort()) {
        ...
        input.onchange=setInputValue(e.target.value);

        //USE THIS BASIC LOGIC TO EXCLUDE OTHER LAYERS FROM BEING EDITED
